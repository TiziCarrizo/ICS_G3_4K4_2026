...
describe('ComprarEntrada', () => {
  ...
  it('debería redirigir a Mercado Pago con formaPago TARJETA sin llamar a la API', async () => {
  ...
  component.actualizarVisitantes();
  component.visitantes.at(0).setValue({ edad: 30, tipoPase: 2 });
  component.form.get('formaPago')?.setValue(2); // TARJETA

  await component.submitForm();

  // No hay request HTTP — TARJETA redirige directamente
  httpMock.expectNone('http://127.0.0.1:8000/api/compras/');
